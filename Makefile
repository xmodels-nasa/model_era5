...
ft:
	...
	nohup python3 -u "$(FT_SCRIPT)" > "$$log_file" 2>&1 & \
...
ft-3x3:
	...
	nohup python3 -u "$(FT_3X3_SCRIPT)" $(FT_3X3_ARGS) > "$$log_file" 2>&1 & \
...
embedding:
	...
	nohup python3 -u "$(EMBEDDING_SCRIPT)" > "$$log_file" 2>&1 & \
...
raw-chips:
	...
	nohup .venv/bin/python -u "$(RAW_CHIPS_SCRIPT)" > "$$log_file" 2>&1 & \
...
baseline-train:
	...
	nohup .venv/bin/python -u "$(BASELINE_TRAIN_SCRIPT)" > "$$log_file" 2>&1 & \
