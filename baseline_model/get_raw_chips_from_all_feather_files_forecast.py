#!/usr/bin/env python3
"""Generate forecast raw ERA5 chips for every Feather file under FEATHER_ROOT.

For a target row at rounded hour t and default lead 6h, the raw-chip context
is built for t-6. The saved labels, row indices, and row timestamps still
point to the target row at t.
"""
...
from build_raw_chips_from_feather_file import (  # noqa: E402
    DATA_ROOT,
    FORECAST_LEAD_HOURS,
    _chip_coverage_note,
...
FORECAST_RAW_CHIPS_DIR = os.getenv(
    ...
    os.getenv("RAW_CHIPS_FORECAST_DIR", str(PROJECT_ROOT / "raw_chips_forecast")),
)
...
def main() -> List[Dict[str, object]]:
    ...
    parser.add_argument("--output-dir", type=str, default=FORECAST_RAW_CHIPS_DIR)
    parser.add_argument(
        "--forecast-lead-hours",
        type=int,
        default=FORECAST_LEAD_HOURS,
        help="Hours between the input context target and the verification target.",
    )
    parser.add_argument(
        "--chip-size",
        type=int,
    ...
    if args.forecast_lead_hours <= 0:
        raise ValueError("--forecast-lead-hours must be > 0.")
    ...
    print(f"Writing forecast raw chips to: {out_dir}")
    print(
        f"chip_size={args.chip_size} | sample_ratio={args.sample_ratio} | "
        f"max_rows={args.max_rows} | forecast_lead_hours={args.forecast_lead_hours}"
    )
